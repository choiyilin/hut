...
export function ProfileClient({ user }: Props) {
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .then(async ({ data, error }) => {
        if (cancelled) return
        if (error) {
          // Fail soft — the dashboard renders without listings rather than
          // crashing the page. Phase 7 surfaces this through an error toast.
          setRealtorListings([])
        ...
        // Backfill is best-effort: a network blip here shouldn't propagate
        // an unhandled rejection (the previous implementation didn't catch).
        try {
          await fetch("/api/geocode/backfill", { method: "POST" })
        } catch {
          // Silent — the listings still render with the un-geocoded rows;
          // the map filter on the listings page already handles 0,0.
        }
      })
    return () => {
      cancelled = true
  ...
  const handleDeleteListing = async (id: string) => {
    const supabase = createClient()
    const { error } = await supabase.from("realtor_listings").delete().eq("id", id)
    if (error) return
